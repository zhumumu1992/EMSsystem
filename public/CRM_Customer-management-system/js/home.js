...
$.ajax({
	type: "get",
	url: "http://localhost:1967/getDataArr/getWorkSchedule",
	...
	error: function() {
		$('.banDel').fadeIn(200);
		$('.banDel .delP1').text('日常工作安排数据加载失败！');
	}
});
...
$('.NewItems').bind('click',function(){
	if($('.PersonInCharge').val() == '' || $('.JobContent').val() == ''){
		...
		$.ajax({
			type: "get",
			url: "http://localhost:1967/AddInfo/NewItems",
			data:{
			...
			error: function() {
				$('.banDel').fadeIn(200);
				$('.banDel .delP1').text('error,新增事项失败！');
			}
		});
	}
})
...
$('body').delegate('.deleteThisWork','click',function(){
	...
	if(sessionStorage.admin_id == '1'){
		...
		$('.yes').bind('click',function(){
			$.ajax({
				type:"get",
				url:"http://localhost:1967/deleteCustomerInfo/deleteThisWork",
				data:{
					id:thisID
				},
				success:function(data){
